...
class CompanyWidget(QWidget):
	...
	def __init__(self, name : str, cost: float, cmp_id : int):
		super().__init__()
		self.id = cmp_id
		self.setFixedSize(570,50)	# Fixed size does not allow to resize this widget from scroll area
		self.group_box = QGroupBox(name, parent = self)
		self.group_box.resize(570,50)
		layout = QHBoxLayout()
		self.group_box.setLayout(layout)
		self.group_box.setStyleSheet(''' font-size: 11px; ''')
		...
		self.buy_button.setFixedSize(60,20)
		self.buy_button.move(480,25)
